...
import org.eclipse.titan.generated.UDPasp__PortType.UDPasp__PT_BASE;
import org.eclipse.titan.generated.UDPasp__Types.ASP__UDP;
import org.eclipse.titan.generated.UDPasp__Types.ASP__UDP__close;
import org.eclipse.titan.generated.UDPasp__Types.ASP__UDP__message;
import org.eclipse.titan.generated.UDPasp__Types.ASP__UDP__open;
import org.eclipse.titan.generated.UDPasp__Types.ASP__UDP__open__result;
...
public class UDPasp__PT extends UDPasp__PT_BASE {
	...
	public void outgoing_send(ASP__UDP send_par) {
		...
		TitanCharString addrf = send_par.getaddressf();
		if (addrf != null) {
			String hostname = addrf.getValue().toString();
			
			TitanInteger portf = send_par.getportf();
			if (portf != null) {
		...
		try {
			TitanOctetString pdu = send_par.getdata();
	...
	public void outgoing_send(ASP__UDP__open send_par) {
		...
		if (send_par.getlocal__addr().is_bound() && send_par.getlocal__addr().is_present()) {
			localAddrStr = send_par.getlocal__addr().get().getValue().toString();
			if (send_par.getlocal__port().is_bound() && send_par.getlocal__port().is_present()) {
		...
		if (send_par.getremote__addr().is_bound() && send_par.getremote__addr().is_present()) {
			remoteAddrStr = send_par.getremote__addr().get().getValue().toString();
			if (send_par.getremote__port().is_bound() && send_par.getremote__port().is_present()) {
	...
	public void outgoing_send(ASP__UDP__close send_par) {
		...
		int dcId = send_par.getid().getInt();
	...
	public void outgoing_send(ASP__UDP__message send_par) {
		...
		if (titanId.is_bound() && titanId.is_present()) { //Channel ID is set
		...
		} else //Channel ID not set, "emulating" simple mode
		{
			try {
				dc = DatagramChannel.open();
				ASP__UDP asp_udp = new ASP__UDP(send_par.getdata(), send_par.constGetremote__addr().get(), send_par.getremote__port().get());
